...
Right: a schematic neuron map of those cells (DNp01, DNp09, MDN, LPLC2, MN9, and the rest of the default atlas). Click a firing cell and bind it to a control.
...
2. Neuron map: region-jitter positions, not EM reconstruction coordinates. Gold is rate. Click binds.
...
Atlas source is `schematic`. Overlay real xyz with an `atlas` object in the pack (`mergeAtlas`).
