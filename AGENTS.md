...
`fly_shell` is an empty chassis: 3D fly on a grid, schematic neuron map, mapping table, plug slot. The default atlas uses FlyWire / MaleCNS type labels. Positions are schematic region jitter, not EM coordinates.
